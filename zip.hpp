...
public:
    _zip(T from, E to) : _iteratable_A(from), _iteratable_B(to) {} 

    auto begin() const{ 
        return  _zip::iterator<decltype(_iteratable_A.begin()),decltype(_iteratable_B.begin())>(_iteratable_A.begin(), _iteratable_B.begin()); }  

    auto end() const {
        return _zip::iterator<decltype(_iteratable_A.end()),decltype(_iteratable_B.end())>(_iteratable_A.end(), _iteratable_B.end()); }   
};  
